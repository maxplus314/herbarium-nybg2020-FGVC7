...
"""
  Modified https://github.com/tensorflow/models/blob/ \
    3ef7bbcf162e71ee77e6466a3eb5c175748ea761/research/autoaugment/ \
    augmentation_transforms.py
  Motivated by 'AutoAugment: Learning Augmentation Policies from Data' [arxiv.org/abs/1805.09501]
...
def zero_pad_and_crop(img, amount=4):
  ...
  Returns:
    ...
    shape as `img`.
  """
  amountr = from32(amount, pil_img.height)
  amountc = from32(amount, pil_img.width)
  padded_img = np.zeros((img.shape[0] + amountr * 2, img.shape[1] + amountc * 2,
                         img.shape[2]))
  padded_img[amountr:img.shape[0] + amountr, amountc:
             img.shape[1] + amountc, :] = img
  top = np.random.randint(low=0, high=2 * amountr)
  left = np.random.randint(low=0, high=2 * amountc)
...
def from32(x, side):
  return randround(from32f(x, side))
...
def create_cutout_mask(img_height, img_width, num_channels, size):
  ...
  sizer = from32(size, img_height)
  sizec = from32(size, img_width)
  upper_coord = (max(0, height_loc - randround(sizer / 2)), max(0, width_loc - randround(sizec / 2)))
  lower_coord = (min(img_height, height_loc + randround(sizer / 2)),
                 min(img_width, width_loc + randround(sizec / 2)))
...
def int_parameter(level, maxval):
  ...
  """
  return randround(level * maxval / PARAMETER_MAX)
...
def pil_unwrap(pil_img):
  ...
  pic_array = (np.array(pil_img) / 255.0)
...
def _translate_x_impl(pil_img, level):
  ...
  """
  level = from32f(level, pil_img.width)
...
def _translate_y_impl(pil_img, level):
  ...
  """
  level = from32f(level, pil_img.height)
  level = int_parameter(level, 10)
  if random.random() > 0.5:
    level = -level
...
def _crop_impl(pil_img, level, interpolation=Image.BILINEAR):
  ...
  levelr = from32(level, pil_img.height)
  levelc = from32(level, pil_img.width)
  cropped = pil_img.crop((level, level, pil_image.width - levelc, pil_image.height - levelr))
  resized = cropped.resize((pil_image.width, pil_image.height, interpolation))
  return resized
